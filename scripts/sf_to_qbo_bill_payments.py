...
def create_qbo_bill_payment(access_token, realm_id, batch, invoices, bank_account_id):
    ...
    vendor_qbo_id = (batch.get("Supplier__r") or {}).get("QBO_Id__c")
    ...
    for inv in invoices:
        qbo_bill_id = inv.get("QBO_Bill_Id__c")
        if not qbo_bill_id:
            raise ValueError(f"Invoice {inv.get('Id')} has no QBO_Bill_Id__c — cannot link to bill payment")
        amount = inv.get("Amount__c") or 0
        lines.append({
            "Amount": amount,
            "LinkedTxn": [{"TxnId": str(qbo_bill_id), "TxnType": "Bill"}],
        })
        total += amount
...
def get_pending_batches(sf):
    result = sf.query(
        "SELECT Id, Name, Date__c, Supplier__r.QBO_Id__c "
...
def get_batch_invoices(sf, batch_id):
    result = sf.query(
        f"SELECT Id, QBO_Bill_Id__c, Amount__c "
        f"FROM Supplier_Invoice__c "
        f"WHERE Payment_Batch__c = '{batch_id}' "
        f"AND QBO_Bill_Id__c != null"
    )
    return result.get("records", [])
...
def main():
    ...
    for batch in batches:
        ...
        try:
            ...
            print(f"  OK    | {batch_name} | {len(invoices)} invoice(s) → QBO Bill Payment {payment_id}")
            sent += 1
